...
import com.bosnet.ngemart.libgen.barcode_scanner.camera.CameraManager;
...
public final class AmbientLightManager implements SensorEventListener {
    ...
    private static final float BRIGHT_ENOUGH_LUX = 40.0f;
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (cameraManager != null) {
            if (ambientLightLux <= TOO_DARK_LUX) {
                setTorch(true);
                torchListener.setChangeTorch(true);
            } else if (ambientLightLux >= BRIGHT_ENOUGH_LUX) {
                setTorch(false);
                torchListener.setChangeTorch(false);
            }
        }
        Log.d("ambientLightLux:",Float.toString(ambientLightLux));
    }
