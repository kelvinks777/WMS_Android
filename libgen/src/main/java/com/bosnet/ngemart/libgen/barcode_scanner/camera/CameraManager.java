...
import com.bosnet.ngemart.libgen.barcode_scanner.Size;
import com.bosnet.ngemart.libgen.barcode_scanner.SourceData;
import com.bosnet.ngemart.libgen.barcode_scanner.client.AmbientLightManager;
import com.bosnet.ngemart.libgen.barcode_scanner.client.camera.CameraConfigurationUtils;
import com.bosnet.ngemart.libgen.barcode_scanner.client.camera.open.OpenCameraInterface;
 ...
 * 7. close()
 *
 * fix calculateFocusArea from :http://baxincc.cc/questions/317798/android-setfocusarea-and-auto-focus
 * reference area rect : http://stackoverflow.com/questions/17996696/in-android-what-is-really-the-use-for-focusing-of-the-weight-area-parameter
 */
public final class CameraManager {
    ...
    public void focusOnTouch(MotionEvent ev) {
        if (camera != null ) {
            ...
            if (parameters.getMaxNumMeteringAreas() > 0){
                ...
                Log.i(TAG,"fancy !");
                Rect meteringRect  = calculateFocusArea(ev.getX(),ev.getY());
                Rect focusRect  = calculateFocusArea(ev.getX(),ev.getY());
                ...
                Log.d(TAG, "focusOnTouch: "+Boolean.toString(isTorchOn())+"- param:"+parameters.getFlashMode());
                camera.setParameters(parameters);
    ...
    //region 'calculateFocusArea'
    private Rect calculateFocusArea(float x, float y) {
        int left = clamp(Float.valueOf((x / getPreviewSize().width * 2000 - 1000)).intValue(), FOCUS_AREA_SIZE);
        int top = clamp(Float.valueOf((y / getPreviewSize().height * 2000 - 1000)).intValue(), FOCUS_AREA_SIZE);
